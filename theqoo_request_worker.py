...
while True:
    ...
    # W17m 직장인 게시판
    # USlJ 중계 게시판
    req = s.get('https://theqoo.net/ktalk?filter_mode=normal',headers=headers)
    ...
    # print(bs)
    # print(bs.find_all('td','title'))
    i = 0 
    # for span in bs.find_all('td','title'):
    #     i+=1
    #     if(i>5):
    #         print(span.text.split("\n")[1])
            # print(span.find('a',class_="replyNum"))
        # if(span.find(class="공지") or span.find(text="필독")) : 
        #     continue
        # else :
    ...
    f = io.open('theqoo_woker_bak.txt', mode='w', encoding='utf-8')
    i=0
    for span in bs.find_all('td','title'):
        if(i>5):
            fStr = span.text.split("\n")[1]
            f.write(fStr+"\n")
        i+=1
    f.close()
    ## 현재 마지막으로 가져온 게시글과 새로가져온 게시글 목록 비교를 위해 cafe_woker.txt를 배열로 가져옴
    f1 = io.open('theqoo_woker.txt', mode='r', encoding='utf-8')
    ...
    f = io.open('theqoo_woker_bak.txt', mode='r', encoding='utf-8')
    allData = f.readlines()
    index = -1
    for line in allData : 
        index = index+1
    ...
    f1 = io.open('theqoo_woker.txt', mode='a', encoding='utf-8')
